...
import com.uchicom.pj.factory.di.DIFactory;
...
public class Context {
  ...
  private Context() {
    try {
      // Hikari設定
      logger.info(System.getProperty("com.uchicom.pj.db", "src/main/resources/db.properties"));
      dbProperties =
          ResourceUtil.createProperties(
              new File(System.getProperty("com.uchicom.pj.db", "src/main/resources/db.properties")),
              "UTF-8");
      // 設定
      logger.info(
          System.getProperty("com.uchicom.pj.config", "src/main/resources/config.properties"));
      configProperties =
          ResourceUtil.createProperties(
              new File(
                  System.getProperty(
                      "com.uchicom.pj.config", "src/main/resources/config.properties")),
              "UTF-8");
    } catch (Exception e) {
