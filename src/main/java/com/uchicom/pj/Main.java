...
public class Main {
  ...
  void execute(ZouniParameter zouniParameter) throws Exception {
    var servlet = new RootServlet(logger, "./www/", "pj/user.htm");
    var map = new HashMap<String, Servlet>();
    map.put("pub./pj/user/", servlet);
    map.put("pub./pj/js/validation.js", new JsServlet(jsFactory::createValidationPage));
    var startWithMap = new HashMap<String, Servlet>();
    startWithMap.put("pub./pj/user/", servlet);
    startWithMap.put("pub./pj/api/", apiServlet);
